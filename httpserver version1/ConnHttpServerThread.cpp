...
QString ConnHttpServerThreadPrivate::doPostJson(Json::Value json)
{
    ...
    if (json.size() > 0)
    {
        ...
        qDebug() << "POST_JSON_DEBUG: Prepared JSON string (first 100 chars): " << QString::fromStdString(jsonStr).left(100);
    }
    ...
    qDebug() << "POST_JSON_DEBUG: curl command (first 100 chars): " << QString::fromStdString(cmd).left(100);
...
bool ConnHttpServerThreadPrivate::doNextMessage()
{
	...
	if (reader.parse(msg.toStdString(), root))
	{
		...
		{
			threadDelay = atoi(interval.c_str());
		}
